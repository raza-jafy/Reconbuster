...
cat target-wildcards.txt | gau --subs --threads 16 | anew gau-urls.txt | uro | anews unique-urls.txt
...
process_document() {
    ...
    if grep -q "internal use only|confidential" temp.txt; then
        echo "Found sensitive text in: $url && notify -silent -id Confidential-Exif | tee document-finding.txt"
    fi
    
    # Run exiftool on the document
    exiftool "$file" | grep -E "Latitude|Longitude|GPSLocation|comments|passwords|password|URL" && echo "\nFound metadata in: $url && notify -silent -id Confidential-Exif | tee exif-finding.txt"
